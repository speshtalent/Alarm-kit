    ...
    struct AddAlarmView: View {
        ...
        private var fireDate: Date {
            ...
            if repeatType == "monthly" {
                ...
                baseDate = (editingAlarmID == nil && !Calendar.current.isDateInToday(selectedDate)) ? selectedDate : Date()
            }

            var components = Calendar.current.dateComponents(in: TimeZone.current, from: baseDate)
            components.hour = hour24
            components.minute = selectedMinute
            components.second = 0
            var date = Calendar.current.date(from: components) ?? Date()
            if date < Date().addingTimeInterval(-60) && repeatDays.isEmpty && repeatType != "monthly" && Calendar.current.isDateInToday(selectedDate) {
        ...
        struct MonthlyRepeatSheet: View {
            ...
            private var maxDaysForSelectedMonth: Int {
                ...
                let minDays = selectedMonthNumbers.map { month -> Int in
                    switch month {
                    case 2: return 29
                        ...
                        ScrollView {
                            VStack(spacing: 20) {
                                ...
                                ZStack {
                                            ...
                                            ForEach(1...31, id: \.self) { day in
                                                ...
                                                Button {
                                                    ...
                                                    Text("\(day)")
                                                        .font(.system(size: 13, weight: .semibold, design: .rounded))
                                                        .foregroundStyle(
                                                            (!isAvailable || isPastDay) ? Color("SecondaryText").opacity(0.3) :
                                                            selectedDay == day ? .black : Color("PrimaryText")
                                                        )
                                                        .frame(width: 36, height: 36)
                                                        .background(
                                                            (!isAvailable || isPastDay) ? Color.clear :
                                    ...
                                    .animation(.spring(response: 0.3, dampingFraction: 0.7), value: maxDaysForSelectedMonth)
                                    .padding(16)
                                }
                                .padding(.horizontal, 20)
