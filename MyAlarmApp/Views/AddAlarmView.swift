...
struct AddAlarmView: View {
    ...
    var onSave: (Date, String, Bool, TimeInterval) -> Void

    private var fireDate: Date {
        if useSpecificDate {
            // Use exact date + time from date picker
            ...
            // Use today or tomorrow based on hour/minute
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 20) {
                ...
                // Toggle between time only vs specific date
                ZStack {
                ...
                // Save Button
                Button {
                    onSave(fireDate, title, snoozeEnabled, TimeInterval(snoozeDuration * 60))
                    dismiss()
                } label: {
                    Text("Set Alarm")
